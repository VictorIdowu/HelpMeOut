    ...
    return (
      <div className="px-6 bg-white pb-2">
        ...
        <div className="grid md:grid-cols-3 my-16 gap-5 md:px-12 md:pb-8 ">
          {items.map((item) => (
            <div key={item.id} className="service-card flex flex-col justify-center items-center p-6 text-center m-bottom">
                  <span className='text-white bg w-12 h-12 pt-2 text-xl font-semibold rounded-full mb-4'>{item.id}</span>
                  <h2 className="text-xl mb-4 bluetext font-semibold">{item.heading}</h2>
              <p className="text-base greytext mb-5">
                {item.text}
              </p>
